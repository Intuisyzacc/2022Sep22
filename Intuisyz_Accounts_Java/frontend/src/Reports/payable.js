              ...
              <div className="row" style={{ paddingLeft: '22px' }}>
                <div className="col-sm-12">
                  <div className="box box-color box-bordered">
					...
					if(@$searchTime=='')
					{
					$sql=mysqli_query($myconn,"SELECT * FROM `account_pending_v3` WHERE `status`='2' AND `type`='Voucher'");
					}
					if(@$searchTime=='1')
					{
					$sql=mysqli_query($myconn,"SELECT * FROM `account_pending_v3` WHERE `status`='2' AND `type`='Voucher' AND (`tran_Date` BETWEEN '$start' AND '$end')");	
					}
					while($row=mysqli_fetch_array($sql))
					 {
						 $transactionID=$row['transactionID'];
						 $transactionDate=$row['tran_Date'];
						 	$party=getLedgerID($row['sundry']);
						 $partyName=$party['ledger_name'];
					...
					?*/}
                          <tr>
                            ...
                            <td>
                              <a href="pay_voucher.php?id=<?php echo $row['transactionID']; ?>">
                                <button className="btn">Close Now</button>
                              </a>
                              <a href="payable_voucher.php?id=<?php echo $row['transactionID']; ?>">
                                <button className="btn">Voucher</button>
                              </a>
                              <a href="edit_payables.php?id=<?php echo $row['transactionID']; ?>">
                                <button className="btn">Edit</button>
                              </a>
                              <a
                                href="delete_payable.php?id=<?php echo $row['transactionID']; ?>"
